...
export class FlutterwaveService {
  ...
  async createSubaccount(input: {
    ...
    const response = await this.request('/subaccounts', { method: 'POST', body: payload });
  ...
  async refundTransaction(transactionId: string, amount: number) {
    if (!this.configured) {
      return { id: `mock-refund-${transactionId}`, amount, status: 'completed' };
    }

    const response = await this.request(`/transactions/${transactionId}/refund`, {
      method: 'POST',
      body: { amount },
    });
    return response.data;
  ...
  private async request(path: string, options: FlutterwaveRequestOptions = {}) {
    ...
    if (!response.ok || json?.status === 'error') {
      throw new BadGatewayException(json?.message || 'Flutterwave request failed.');
    }
