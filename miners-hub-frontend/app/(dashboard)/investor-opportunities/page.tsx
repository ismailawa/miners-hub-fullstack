...
function esgLine(opportunity: InvestorOpportunity) {
  ...
  if (!summary || summary.total === 0) return "ESG obligations not linked";
...
export default function InvestorOpportunitiesPage() {
  ...
  const stats = useMemo(
    () => ({
      published: opportunities.filter((item) => item.status === "published")
        .length,
      totalCapital: opportunities.reduce(
        (sum, item) => sum + Number(item.capitalRequired || 0),
        0,
      ),
      inquiries: opportunities.reduce(
        (sum, item) => sum + Number(item.inquiryCount || 0),
        0,
      ),
    }),
    [opportunities],
  ...
  const saveOpportunity = async (event: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        capitalRequired: form.capitalRequired
          ? Number(form.capitalRequired)
          : null,
  ...
  const publishSelected = async () => {
    ...
    setSaving(true);
    try {
      const updated = await updateInvestorOpportunity(selected.id, {
        status: "published",
      });
      setSelected(updated);
      await loadOpportunities();
  ...
  const reviewSelected = async (
    reviewStatus: "approved" | "action_required" | "rejected",
  ) => {
    if (!selected) return;
    setSaving(true);
    setError(null);
    try {
      const updated = await reviewInvestorOpportunity(selected.id, {
        reviewStatus,
      });
      setSelected(updated);
      await loadOpportunities();
    } catch (err: any) {
      setError(err?.message || "Failed to review opportunity");
    } finally {
      setSaving(false);
    }
  };
    ...
    <main className="space-y-6">
      <section className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm text-text-secondary">
            Publish investment-ready mining opportunities, review due diligence
            packs, and track investor inquiries.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          <button
            ...
            className="rounded-md border border-border px-4 py-2 text-sm font-semibold text-text-primary hover:bg-border/40"
          >
            Refresh
          </button>
        </div>
      ...
      {canPublish ? (
        <FormModal
          ...
          description="Create an investment-ready opportunity with capital requirements, risk indicators, and due diligence references."
          onClose={() => setIsOpportunityFormOpen(false)}
          maxWidthClass="max-w-4xl"
        >
          <form onSubmit={saveOpportunity} className="space-y-3">
            <input
              required
              value={form.title}
              onChange={(event) =>
            ...
            <input
              required
              value={form.mineralFocus}
              onChange={(event) =>
                setForm((current) => ({
                  ...current,
                  mineralFocus: event.target.value,
                }))
              }
              placeholder="Mineral focus, comma separated"
              className="w-full rounded-md border border-border bg-primary px-3 py-2 text-sm text-text-primary"
            />
            <input
            ...
            <div className="grid grid-cols-2 gap-3">
              <select
                ...
                onChange={(event) =>
                  setForm((current) => ({
                    ...current,
                    stage: event.target.value as InvestorOpportunityStage,
                  }))
                }
                className="rounded-md border border-border bg-primary px-3 py-2 text-sm text-text-primary"
              >
                {stages
              ...
              <select
                ...
                onChange={(event) =>
                  setForm((current) => ({
                    ...current,
                    riskRating: event.target
                      .value as InvestorOpportunityRiskRating,
                  }))
                }
                className="rounded-md border border-border bg-primary px-3 py-2 text-sm text-text-primary"
              >
                {risks
                  .filter((risk) => risk !== "all")
                  .map((risk) => (
                    <option key={risk} value={risk}>
                      {label(risk)}
                    </option>
            ...
            />
            <textarea
              value={form.dueDiligenceDocuments}
              onChange={(event) =>
                setForm((current) => ({
                  ...current,
                  dueDiligenceDocuments: event.target.value,
                }))
              }
              rows={3}
              placeholder="Document lines: Title | URL | Type"
              className="w-full rounded-md border border-border bg-primary px-3 py-2 text-sm text-text-primary"
            />
            <textarea
              ...
              onChange={(event) =>
                setForm((current) => ({
                  ...current,
                  riskIndicators: event.target.value,
                }))
              }
              rows={2}
              placeholder="Risk indicators, comma separated"
              className="w-full rounded-md border border-border bg-primary px-3 py-2 text-sm text-text-primary"
            />
            <select
              value={form.status}
              onChange={(event) =>
                setForm((current) => ({
                  ...current,
                  status: event.target.value as InvestorOpportunityStatus,
                }))
              }
              className="w-full rounded-md border border-border bg-primary px-3 py-2 text-sm text-text-primary"
            >
            ...
            <label className="flex items-center gap-2 text-sm text-text-secondary">
              <input
                type="checkbox"
                checked={form.analyticsSubscriptionEnabled}
                onChange={(event) =>
                  setForm((current) => ({
                    ...current,
                    analyticsSubscriptionEnabled: event.target.checked,
                  }))
                }
              />
            ...
            <div className="flex justify-end gap-2 pt-2">
              <button
                type="button"
                onClick={() => setIsOpportunityFormOpen(false)}
      ...
      {selected && canInquire ? (
        <FormModal
          ...
          description={`Send an inquiry for ${selected.title}.`}
        ...
        >
          <form onSubmit={submitInquiry} className="space-y-3">
            <textarea
              ...
              rows={4}
              placeholder="Investment interest, questions, or diligence request"
              className="w-full rounded-md border border-border bg-primary px-3 py-2 text-sm text-text-primary"
            />
            <input
      ...
      <section
        className={`grid grid-cols-1 gap-6 ${selected ? "xl:grid-cols-[1fr_420px]" : ""}`}
      >
        <div className="space-y-4">
          {loading ? (
            <div className="rounded-lg border border-border bg-secondary p-6 text-text-secondary">
          ...
          {opportunities.map((opportunity) => (
            <button
              ...
              className={`block w-full rounded-lg border p-5 text-left transition ${selected?.id === opportunity.id ? "border-accent bg-accent/5" : "border-border bg-secondary hover:border-accent/40"}`}
            >
              <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                <div>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {opportunity.mineralFocus.map((mineral) => (
                      <span
                        key={mineral}
                        className="rounded-full bg-primary px-2.5 py-1 text-xs text-text-secondary"
                      >
                        {mineral}
                      </span>
                    ))}
                ...
                <div className="shrink-0 text-left md:text-right">
                  ...
                  <p
                    className={`mt-1 text-sm font-bold ${scoreClass(opportunity.riskScore)}`}
                  >
                    Score {opportunity.riskScore ?? "TBD"}
                  </p>
                  <span
                    className={`mt-2 inline-flex rounded-full border px-2.5 py-1 text-xs font-semibold capitalize ${riskClass(opportunity.riskRating)}`}
                  >
                    {label(opportunity.riskRating)} risk
                  </span>
                </div>
              </div>
              <div className="mt-4 grid grid-cols-2 gap-3 text-xs text-text-muted md:grid-cols-6">
          ...
          {!loading && opportunities.length === 0 ? (
            <div className="rounded-lg border border-border bg-secondary p-6 text-text-secondary">
              No investor opportunities match these filters.
            </div>
          ) : null}
        </div>
        ...
        {selected ? (
          <aside className="space-y-6">
            <div className="rounded-lg border border-border bg-secondary p-5">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm capitalize text-text-muted">
                    {label(selected.status)} · {label(selected.stage)} ·
                    diligence{" "}
                ...
                <div className="flex flex-wrap justify-end gap-2">
                  {selected && canInquire ? (
                    <button
                      ...
                      className="rounded-md border border-border px-3 py-2 text-xs font-semibold text-text-primary hover:border-accent hover:text-accent"
                    >
                      Inquire
                    </button>
                  ) : null}
                  {canPublish && selected.status !== "published" ? (
                    <button
                      ...
                      disabled={saving}
                      className="rounded-md bg-accent px-3 py-2 text-xs font-semibold text-accent-content"
                    >
                      Publish
                    </button>
                  ) : null}
                  {canReview ? (
                    <>
                      <button
                        type="button"
                        onClick={() => reviewSelected("action_required")}
                        disabled={saving}
                        className="rounded-md border border-border px-3 py-2 text-xs font-semibold text-text-primary hover:border-accent hover:text-accent"
                      >
                        Require Action
                      ...
                      <button
                        ...
                        disabled={saving}
                        className="rounded-md bg-green-500 px-3 py-2 text-xs font-semibold text-white"
                      >
                        Approve Diligence
                      </button>
                    </>
              ...
              <div className="mt-4 rounded-md border border-border bg-primary p-3">
                ...
                {selected.esgSummary && selected.esgSummary.types.length > 0 ? (
                  ...
                  <p className="mt-1 text-xs text-text-muted">
                    Link a mine site with ESG obligations to expose CDA and
              ...
              <div className="mt-4 space-y-2">
                ...
                {selected.dueDiligenceDocuments.map((document) => (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="block rounded-md border border-border bg-primary px-3 py-2 text-sm text-accent hover:border-accent/40"
                  >
                    {document.title}
              ...
              {selected.analyticsSubscriptionEnabled ? (
                <p className="mt-4 rounded-md bg-primary p-3 text-xs text-text-secondary">
                  Investor intelligence subscription hooks enabled for this
                  opportunity.
                </p>
              ) : null}
            </div>
                  ...
                  {selected.inquiries.map((item) => (
                    <div key={item.id} className="rounded-md bg-primary p-3">
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <select
                          ...
                          onChange={async (event) => {
                            await updateInvestorOpportunityInquiry(item.id, {
                              status: event.target.value as any,
                            });
                            setSelected(
                              await getInvestorOpportunity(selected.id),
                            );
                          }}
                          className="rounded-md border border-border bg-secondary px-2 py-1 text-xs text-text-primary"
                        >
                          {["new", "contacted", "due_diligence", "closed"].map(
                            (status) => (
                              <option key={status} value={status}>
                                {label(status)}
                              </option>
                      ...
                      <p className="mt-2 text-xs text-text-muted">
                        {item.investmentRange || "Range not supplied"} ·{" "}
                        {item.contactPreference || "No contact preference"}
                      </p>
                    </div>
                  ))}
                </div>
