...
export default function AdminOrdersPage() {
  ...
  if (loading) return <BrandLoader label="Loading orders" />;
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-text-secondary">
            Monitor purchases, payment status, and fulfillment progress.
          </p>
        </div>
      ...
      {error && (
        <div className="bg-red-500/10 border border-red-500/50 text-red-400 p-3 rounded-lg text-sm">
          {error}
          <button onClick={() => fetchOrders()} className="underline ml-2">
            Retry
          </button>
        </div>
      )}
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody>
            {orders.map((order) => {
              ...
              const payoutReady =
                escrow?.sellerPayoutAccount?.status === "active";
              const canRelease =
                escrow?.status === "awaiting_release" && payoutReady;
              const canRefund =
                escrow &&
                !["released", "release_processing", "refunded"].includes(
                  escrow.status,
                );
              return (
                <tr
                  key={order.id}
                  className="border-b border-border hover:bg-primary/30 transition-colors align-top"
                >
                  <td className="p-4">
                    <p className="font-medium text-text-primary">
                      {order.listing?.mineralType || "Mineral"}
                    </p>
                  ...
                  <td className="p-4 text-sm text-text-secondary">
                    {order.seller?.name || order.seller?.email || "Seller"}
                  </td>
                  ...
                  <td className="p-4 min-w-56">
                    {escrow ? (
                      <div className="space-y-1 text-xs text-text-secondary">
                        ...
                        <p>
                          Commission: {formatCurrency(escrow.commissionAmount)}
                        </p>
                        <p>
                          Seller net: {formatCurrency(escrow.sellerNetAmount)}
                        </p>
                        <p
                          className={
                            payoutReady ? "text-green-400" : "text-yellow-400"
                          }
                        >
                          Payout:{" "}
                          {escrow.sellerPayoutAccount?.status || "missing"}
                        </p>
                      </div>
                    ) : (
                  ...
                  <td className="p-4">
                    <div className="flex flex-col gap-2 min-w-28">
                      <button
                        ...
                        disabled={
                          !canRelease || actioning === `release-${order.id}`
                        }
                      ...
                      <button
                        ...
                        disabled={
                          !canRefund || actioning === `refund-${order.id}`
                        }
