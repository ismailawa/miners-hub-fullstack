...
const LoginPage: React.FC = () => {
  ...
  const verifyOtpCode = async (code: string) => {
    if (code.length !== 6 || isLoading || lastVerifiedOtpRef.current === code)
      return;
  ...
  return (
    <div className="min-h-screen bg-primary text-text-secondary flex">
      ...
      <div
        ...
        style={{
          backgroundImage: `url('https://picsum.photos/seed/authpage/1200/1800')`,
        }}
      >
        ...
        <div className="relative z-10 text-center text-white">
          ...
          >
            Access Verified Mineral Infrastructure
          </h1>
          <p className="text-lg">
            Sign in to manage compliant trade, due diligence, escrow,
            traceability, logistics, and investment workflows.
          </p>
        </div>
      </div>
        ...
        <div className="w-full max-w-md">
          ...
          {mode === "forgot" && (
            <form onSubmit={handleForgotPassword} className="space-y-4">
              ...
              <button
                ...
                className="w-full text-sm font-semibold text-text-secondary hover:text-accent"
              >
                Back to sign in
              </button>
            </form>
          ...
          {mode === "verify" && (
            <form onSubmit={handleVerifyOtp} className="space-y-4">
              ...
              <button
                type="button"
                onClick={() => void sendPasswordResetOtp()}
                disabled={isLoading}
                className="w-full text-sm font-semibold text-text-secondary hover:text-accent disabled:opacity-50"
              >
                Resend OTP
              </button>
              <button
